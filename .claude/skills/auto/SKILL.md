...
7. **마감 D-day 당일에는 새 아키텍처 금지** — 안정성 우선 (stability runs, seed ensemble만)
8. **오늘 quota 0개면 새 candidate는 internal로만 보관, 다음날 제출 우선순위 정렬
...
**Time State Check** (ALWAYS run before planning):
```bash
python scripts/check_time_state.py
```
이 출력에서 반드시 인식할 것:
- **현재 시각** (KST)
- **예선/본선/포스터 마감까지 남은 일수**
- **오늘 사용한 제출 횟수 / 10회 한도**
- 마감이 3일 이내면 ⚠️, 1일 이내면 🔥 — 전략을 보수적으로 전환
- 오늘 quota가 0이면 새 candidate 만들어도 제출 불가 — 다음날 대비
...
- `data_docs/` 디렉토리가 있으면 모두 읽기 (데이터셋 생성 방법, opensource 출처)
- dev agent에게 위임할 때 이 내용을 컨텍스트로 전달

Print a situational summary:
```
[TIME] Now: {iso} | 예선까지 {N}d | 오늘 제출 {used}/10
...
[DATA] {data_docs 요약 한 줄 또는 "no data_docs"}
