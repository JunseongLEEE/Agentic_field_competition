...
# ===========================================================================
# exp_014 additions: multi-modal fusion inputs
#   (1) text pair for encoder tower
#   (2) history STEP sequence for the trajectory transformer tower
#   (3) split cats / nums for the tabular embedding tower
# ===========================================================================

# --- step-sequence vocab (trajectory tower) --------------------------------
STEP_LIST = ["<PAD>", "<BOS>", "<USER>"] + CLASS_ORDER  # 0,1,2, 3..16
STEP2ID = {t: i for i, t in enumerate(STEP_LIST)}
N_STEP_VOCAB = len(STEP_LIST)          # 17
MAX_SEQ_LEN = 16                       # BOS + up to 15 recent history elements
MAX_TEXT_LEN = 96

# role ids: 0 pad, 1 user, 2 action, 3 bos
# fail ids: 0 pad/na, 1 ok/not-failed, 2 failed


def phase_of(turn_index):
    if turn_index <= 2:
        return "early"
    if turn_index <= 4:
        return "mid"
...
def extract_text_pair(sample):
    """text_a = tagged current prompt; text_b = most-recent user msg in history."""
    prompt = get_prompt(sample)
    history = sample.get("history", []) or []
    actions = _action_list(history)
    last = actions[-1].get("name", NONE_TOKEN) if actions else NONE_TOKEN
    meta = sample.get("session_meta", {}) or {}
    ws = meta.get("workspace", {}) or {}
    ci = _safe_str(ws.get("last_ci_status", NONE_TOKEN)) or NONE_TOKEN
    turn_index = int(meta.get("turn_index", 0) or 0)
    phase = phase_of(turn_index)
    recent_user = ""
    for h in reversed(history):
        if isinstance(h, dict) and h.get("role") == "user":
            recent_user = _safe_str(h.get("content", ""))
            break
    text_a = f"[LAST={last}] [TURN={phase}] [CI={ci}] {prompt}"
    return text_a, recent_user


def extract_seq(sample):
    """Ordered step sequence over history. Returns (tokens, roles, fails)."""
    history = sample.get("history", []) or []
    toks = [STEP2ID["<BOS>"]]
    roles = [3]
    fails = [0]
    for h in history:
        ...
        r = h.get("role")
        if r == "user":
            toks.append(STEP2ID["<USER>"])
            roles.append(1)
            fails.append(0)
        elif r == "assistant_action":
            name = h.get("name", NONE_TOKEN)
            toks.append(STEP2ID.get(name, STEP2ID["<BOS>"]))
            roles.append(2)
            rs = _safe_str(h.get("result_summary", ""))
            fails.append(2 if ("ERROR" in rs.upper() or "FAIL" in rs.upper()) else 1)
    if len(toks) > MAX_SEQ_LEN:
        toks = [toks[0]] + toks[-(MAX_SEQ_LEN - 1):]
        roles = [roles[0]] + roles[-(MAX_SEQ_LEN - 1):]
        fails = [fails[0]] + fails[-(MAX_SEQ_LEN - 1):]
    return toks, roles, fails


def build_multimodal(samples):
    """Return ids, prompts, records, text_pairs, seqs (parallel lists)."""
    ids, prompts, records, text_pairs, seqs = [], [], [], [], []
    for s in samples:
        ids.append(_safe_str(s.get("id", "")))
        prompts.append(get_prompt(s))
        records.append(extract_record(s))
        text_pairs.append(extract_text_pair(s))
        seqs.append(extract_seq(s))
    return ids, prompts, records, text_pairs, seqs


# --- tabular tower: separate categorical indices / numeric values ----------

def records_to_cats(records, cat_mappings):
    rows = []
    for r in records:
        rows.append([_encode_cat(r[f], cat_mappings[f]) for f in CAT_FIELDS])
    return np.asarray(rows, dtype=np.int64)


def records_to_nums(records):
    rows = []
    for r in records:
        row = [float(r.get(f, 0) or 0) for f in NUM_FIELDS]
        row += [float(r.get("cnt_" + c, 0)) for c in CLASS_ORDER]
        row += [float(x) for x in r["_rule_flags"]]
        rows.append(row)
    return np.asarray(rows, dtype=np.float32)


def num_feature_dim():
    return len(NUM_FIELDS) + len(CLASS_ORDER) + len(RULE_NAMES)


def cat_cardinalities(cat_mappings):
    return [len(cat_mappings[f]) for f in CAT_FIELDS]
