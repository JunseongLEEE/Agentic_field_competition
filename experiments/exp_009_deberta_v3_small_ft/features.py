...
# ---------------------------------------------------------------------------
# Structured-feature serialization for encoder fine-tuning (exp_009)
# ---------------------------------------------------------------------------

def _last_user_message(sample):
    """Return the most recent history user-turn content (or '')."""
    history = sample.get("history", []) or []
    if not isinstance(history, list):
        return ""
    for h in reversed(history):
        if isinstance(h, dict) and h.get("role") == "user":
            return _safe_str(h.get("content", ""))
    return ""


def build_input_text(sample, rec, prompt, max_prev_chars=200):
    """Serialize compact structured fields + current_prompt into one input string.

    Layout (carries the dominant structured signal — last_action etc.):
      tier=.. lang=.. turn=.. last=.. prev=.. failed=0/1 ci=.. nopen=.. hist=..
      [SEP] {current_prompt} [SEP] prev_user: {last user message}
    """
    struct = (
        f"tier={rec['user_tier']} lang={rec['language_pref']} "
        f"turn={int(rec['turn_index'])} last={rec['last_action']} "
        f"prev={rec['second_last_action']} failed={int(rec['last_action_failed'])} "
        f"ci={rec['last_ci_status']} nopen={int(rec['n_open_files'])} "
        f"hist={int(rec['history_len'])}"
    )
    prev_msg = _last_user_message(sample)
    if len(prev_msg) > max_prev_chars:
        prev_msg = prev_msg[:max_prev_chars]
    text = f"{struct} [SEP] {prompt}"
    if prev_msg:
        text = f"{text} [SEP] prev_user: {prev_msg}"
    return text


def build_texts(samples):
    """samples -> (ids, texts) serialized input strings for the encoder."""
    ids, texts = [], []
    for s in samples:
        ...
        rec = extract_seq_meta(s)
        ids.append(sid)
        texts.append(build_input_text(s, rec, p))
    return ids, texts
