#!/usr/bin/env python3
"""Local training + 5-fold stratified CV for exp_002_tfidf_xgboost.

NEVER submitted. Produces:
  - oof_preds.npy (70000, 14) float32
  - test_preds.npy (n_test, 14) float32
  - model/  (word_vec, char_vec, cat_mappings, class_order, xgb full-train model)
  - models/ (per-fold xgb models for diagnostics)
  - train_log.json
...
def main():
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(np.zeros(len(y_codes)), y_codes)):
        ...
        joblib.dump(model, models_dir / f"xgb_fold{fold}.pkl")
        best_it = getattr(model, "best_iteration", None)
        print(f"[fold {fold}] macro_f1={fs:.4f} best_iter={best_it} "
    ...
    joblib.dump(model_full, model_dir / "xgb.pkl")
