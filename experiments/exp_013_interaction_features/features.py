...
RUN_VERB_RE = re.compile(
    r"(돌려|돌려봐|돌려보자|돌려서|돌려줘|실행|run\b|rerun|execute|빌드)", re.I)
LINT_KEYWORD_RE = re.compile(
    r"(lint|typecheck|타입체크|타입|정적분석|fmt|format|shellcheck|mypy|ruff|flake8)", re.I)
ERROR_IN_PROMPT_RE = re.compile(
    r"(TypeError|AttributeError|ImportError|KeyError|RuntimeError|ValueError|"
    r"ConnectionError|AssertionError|SyntaxError|NameError|Exception|에러|오류)", re.I)
...
CAT_FIELDS = [
    ...
    "prompt_len_bucket",
    # Priority 1: 돌려봐 3파전 interaction
    "run_verb_x_last",
    "lint_kw_x_last",
    # Priority 2: 탐색 4형제
    "explore_bigram",
...
NUM_FIELDS = [
    ...
    # Priority 1: 돌려봐 3파전
    "is_run_verb", "is_lint_keyword",
    "run_verb_x_failed",
    # Priority 2: 탐색 4형제
    "explore_streak", "explore_depth",
    # Priority 3: plan vs ask
    "has_error_in_prompt",
    "question_x_no_history", "question_x_early_turn",
]
...
def extract_record(sample):
    ...
    # === Priority 1: 돌려봐 3파전 features ===
    is_run_verb = 1 if RUN_VERB_RE.search(prompt) else 0
    is_lint_kw = 1 if LINT_KEYWORD_RE.search(prompt) else 0
    run_verb_x_last = f"run_{last}" if is_run_verb else "no_run"
    lint_kw_x_last = f"lint_{last}" if is_lint_kw else "no_lint"
    run_verb_x_failed = is_run_verb * last_failed

    # === Priority 2: 탐색 4형제 features ===
    explore_streak = 0
    for a in reversed(action_names):
        if a in EXPLORE_ACTIONS:
            explore_streak += 1
        else:
            break
    explore_depth = sum(1 for a in action_names if a in EXPLORE_ACTIONS)
    last_explore_type = last if last in EXPLORE_ACTIONS else NONE_TOKEN
    if last in EXPLORE_ACTIONS and second in EXPLORE_ACTIONS:
        explore_bigram = f"{second}__{last}"
    else:
        explore_bigram = NONE_TOKEN

    # === Priority 3: plan vs ask features ===
    has_error_in_prompt = 1 if ERROR_IN_PROMPT_RE.search(prompt) else 0
    prompt_has_q = 1 if "?" in prompt else 0
    error_x_question_cat = "err_q" if (has_error_in_prompt and prompt_has_q) else (
        "err_noq" if has_error_in_prompt else (
        "noerr_q" if prompt_has_q else "noerr_noq"))
    question_x_no_history = prompt_has_q * (1 if len(action_names) == 0 else 0)
    question_x_early_turn = prompt_has_q * (1 if turn_index <= 2 else 0)
    ...
    out = {
        ...
        # Priority 1: 돌려봐 3파전 categoricals
        "run_verb_x_last": run_verb_x_last,
        "lint_kw_x_last": lint_kw_x_last,
        # Priority 2: 탐색 4형제 categoricals
        "explore_bigram": explore_bigram,
        "last_explore_type": last_explore_type,
        # Priority 3: plan vs ask categoricals
        "error_x_question": error_x_question_cat,
        # Numerics
        ...
        # Priority 1: 돌려봐 3파전 numerics
        "is_run_verb": is_run_verb,
        "is_lint_keyword": is_lint_kw,
        "run_verb_x_failed": run_verb_x_failed,
        # Priority 2: 탐색 4형제 numerics
        "explore_streak": explore_streak,
        "explore_depth": explore_depth,
        # Priority 3: plan vs ask numerics
        "has_error_in_prompt": has_error_in_prompt,
        "question_x_no_history": question_x_no_history,
        "question_x_early_turn": question_x_early_turn,
    }
    out.update(counts)
